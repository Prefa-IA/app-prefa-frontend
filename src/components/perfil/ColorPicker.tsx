...
import { hexToRgb, rgbToHex, parseColor } from '../../utils/colorUtils';

const ColorPicker: React.FC<ColorPickerProps> = ({ label, description, value, onChange, disabled }) => {
  ...
  const emitChange = (newHex: string, newAlpha: number) => {
    ...
    const cssColor = newAlpha >= 1 ? newHex : `rgba(${rgb.r},${rgb.g},${rgb.b},${newAlpha.toFixed(2)})`;
    onChange(cssColor);
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center space-x-1 sm:space-x-4">
        <div
          ...
          style={{ backgroundColor: alpha >= 1 ? hex : (() => { const rgb = hexToRgb(hex); return `rgba(${rgb.r},${rgb.g},${rgb.b},${alpha})`; })() }}
        />
        <input
          type="color"
          value={hex}
