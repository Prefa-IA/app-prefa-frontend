...
export const processImageFiles = (fileList: FileList | File[], existingImages: File[]): { valid: File[]; previews: Promise<string[]> } => {
  ...
  const previews = Promise.all(
    limited.map(file => new Promise<string>((resolve) => {
