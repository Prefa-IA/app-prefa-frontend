...
export const generateIndexContext = (
  ...
  return {
    ...
    fachadaImages,
    documentosVisuales,
    hasEdificabilidad: !!(informeAMostrar.edificabilidad && Object.keys(informeAMostrar.edificabilidad).length > 0),
    hasGeometria: !!(informeAMostrar.geometria && informeAMostrar.geometria.features?.length > 0),
    hasEntorno: !!(informeAMostrar.entorno && (
    ...
    hasTroneras: !!(informeAMostrar.edificabilidad?.troneras && informeAMostrar.edificabilidad.troneras.cantidad > 0),
    hasAPH: !!(informeAMostrar.edificabilidad?.catalogacion && informeAMostrar.edificabilidad.catalogacion.catalogacion),
    hasLFI: !!(informeAMostrar.edificabilidad?.lfi_disponible && informeAMostrar.edificabilidad.lfi_disponible !== 'No'),
    hasLBI: !!(informeAMostrar.edificabilidad?.longitud_lfi && parseFloat(informeAMostrar.edificabilidad.longitud_lfi) > 0)
  };
};
...
export const getVisibleIndexItems = (items: DynamicIndexItem[]): DynamicIndexItem[] => {
  return items.filter(item => item.visible).map(item => ({
    ...item,
    subItems: item.subItems ? getVisibleIndexItems(item.subItems) : undefined
  }));
};
